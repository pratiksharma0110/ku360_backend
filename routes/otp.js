...
// Validate email format (use regex for better validation)
...
router.post('/verify-otp', (req, res) => {
  ...
  if (storedOtp.otp === otp && storedOtp.expiresAt > Date.now()) {
    delete otpStorage[email]; // OTP is valid, delete it from storage
    return res.send({ success: true, message: 'OTP verified successfully' });
  }

  // OTP either expired or incorrect
